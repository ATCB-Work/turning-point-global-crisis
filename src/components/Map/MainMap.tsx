...
const generateInfrastructureMarkers = (city: City, type: 'airport' | 'port', coords: [number, number]): ReactNode => (
...
const getCustomCoords = (cityName: string, defaultCoords: [number, number], offset: [number, number]): [number, number] => {
    let coords: [number, number] = [
        defaultCoords[0] + offset[0],
        defaultCoords[1] + offset[1]
    ];

    // TODO: Add specific overrides for cityName if needed
    if (cityName === '') {
...
export default function MainMap({ nations, playerNation, activeLayers, selectedNationId, onNationClick }: MainMapProps) {
    ...
    }, [nations, activeLayers.locations]);
